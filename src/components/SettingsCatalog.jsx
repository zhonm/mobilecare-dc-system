...
export default function SettingsCatalog() {
  ...
  const handleExportEgressReport = () => {
    try {
      const reportData = {
        reportType: 'Supabase Free-Tier Egress & Bandwidth Audit',
        generatedAt: new Date().toISOString(),
        project: 'wjikbnbdkxmesvvzbams',
        freeTierLimitGb: 5.0,
        stats: egressStats
      };
      ...
      a.href = url;
      a.download = `mdc_supabase_egress_audit_${new Date().toISOString().slice(0, 10)}.json`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      showToast('Egress audit report exported successfully', 'success');
    } catch (e) {
    ...
    <div className="settings-view" style={{ maxWidth: '1240px', margin: '0 auto', paddingBottom: '30px' }}>
      ...
      {activeTab === 'sql' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {/* Card: Supabase Free-Tier Egress Usage & Quota Defense Monitor */}
          <div
            className="card"
            style={{
              padding: '24px',
              border: egressStats.alertLevel === 'emergency' || egressStats.alertLevel === 'critical'
                ? '1px solid #f87171'
          ...
          >
            {/* Header: Title, Alert Badge & Quick Controls */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px', marginBottom: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div
                  style={{
                    background: egressStats.usagePercent >= 80 ? '#ef4444' : egressStats.usagePercent >= 60 ? '#f59e0b' : '#0284c7',
                    color: '#fff',
                    padding: '10px',
                    borderRadius: '10px',
                    display: 'flex',
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                    <h4 style={{ margin: 0, fontSize: '17px', fontWeight: 800, color: 'var(--text-main)', letterSpacing: '-0.02em' }}>
                      Supabase Free-Tier Egress & Bandwidth Monitor
                    </h4>
                    <span
                      style={{
                        fontSize: '11px',
                        ...
                        padding: '2px 8px',
                        borderRadius: '12px',
                        background: egressStats.alertLevel === 'emergency' || egressStats.alertLevel === 'critical'
                          ? '#fee2e2'
                          : egressStats.alertLevel === 'warning'
                          ? '#fef3c7'
                          : '#dcfce7',
                        color: egressStats.alertLevel === 'emergency' || egressStats.alertLevel === 'critical'
                          ? '#dc2626'
                          : egressStats.alertLevel === 'warning'
                          ? '#d97706'
                          : '#16a34a',
                    ...
                    >
                      {egressStats.alertLevel.toUpperCase()} PACE
                    </span>
                  </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                ...
                <button
                  type="button"
                  className="btn btn-secondary btn-sm"
                  onClick={() => setIsEditingBaseline(prev => !prev)}
                  style={{ display: 'flex', alignItems: 'center', gap: '6px' }}
                >
                  <Sliders size={14} />
                  <span>{isEditingBaseline ? 'Cancel' : 'Calibrate Baseline'}</span>
                </button>

                <button
                  type="button"
                  className="btn btn-secondary btn-sm"
                  onClick={handleExportEgressReport}
                  style={{ display: 'flex', alignItems: 'center', gap: '6px' }}
                >
                  <Download size={14} />
              ...
              >
                <div style={{ flex: 1, minWidth: '220px' }}>
                  <label style={{ display: 'block', fontSize: '11.5px', fontWeight: 700, color: 'var(--text-main)', marginBottom: '4px' }}>
                    Dashboard Recorded Egress Total (in Gigabytes)
                  </label>
                  <input
                    type="number"
                    step="0.01"
                ...
                <div style={{ width: '100%', fontSize: '11.5px', color: 'var(--text-muted)' }}>
                  Enter the total GB shown on your Supabase usage dashboard. Subsequent network requests in the system will automatically accumulate on top of this verified baseline.
                </div>
              </form>
            )}
            ...
            <div style={{ marginBottom: '22px' }}>
              ...
              >
                <div
                  style={{
                    width: `${Math.min(100, egressStats.usagePercent)}%`,
                    height: '100%',
                    background: egressStats.usagePercent >= 80 ? '#ef4444' : egressStats.usagePercent >= 60 ? '#f59e0b' : '#10b981',
                    borderRadius: '8px',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: 'var(--text-muted)', marginTop: '6px' }}>
                ...
                <span>4.0 GB (80% Egress Saver Active)</span>
                <span>5.0 GB Quota Cap</span>
              </div>
            </div>

            {/* 4 Statistical Metric Tiles */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', marginBottom: '22px' }}>
              <div style={{ padding: '14px', background: 'var(--bg-card-alt, #f8fafc)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
              ...
              <div style={{ padding: '14px', background: 'var(--bg-card-alt, #f8fafc)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                ...
                <div style={{ fontSize: '18px', fontWeight: 800, color: egressStats.daysUntilLimit < egressStats.daysRemaining ? '#ef4444' : '#10b981', marginTop: '4px' }}>
                  {egressStats.daysUntilLimit < 999 ? `${Math.round(egressStats.daysUntilLimit)} days` : '30+ days'}
                </div>
                <div style={{ fontSize: '11.5px', color: '#64748b', marginTop: '2px' }}>
                  {egressStats.daysUntilLimit < egressStats.daysRemaining ? 'Quota runs out before reset' : 'Sufficient headroom for cycle'}
                </div>
              </div>
            </div>

            {/* Subtabs for Detail Breakdown */}
            <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '16px' }}>
              <div style={{ display: 'flex', gap: '8px', marginBottom: '14px', borderBottom: '1px solid var(--border-color)', paddingBottom: '8px' }}>
                <button
                  type="button"
                  onClick={() => setEgressViewTab('history')}
                  style={{
                    background: egressViewTab === 'history' ? '#0284c7' : 'transparent',
                    color: egressViewTab === 'history' ? '#fff' : 'var(--text-muted)',
                    border: 'none',
                    borderRadius: '6px',
                    padding: '6px 12px',
                    fontSize: '12px',
                ...
                >
                  Daily Egress Breakdown
                </button>
                <button
                  type="button"
                  onClick={() => setEgressViewTab('tables')}
                  style={{
                    background: egressViewTab === 'tables' ? '#0284c7' : 'transparent',
                    color: egressViewTab === 'tables' ? '#fff' : 'var(--text-muted)',
                    border: 'none',
                    borderRadius: '6px',
                    padding: '6px 12px',
                    fontSize: '12px',
                ...
                >
                  Bandwidth by Table / Service
                </button>
                <button
                  type="button"
                  onClick={() => setEgressViewTab('requests')}
                  style={{
                    background: egressViewTab === 'requests' ? '#0284c7' : 'transparent',
                    color: egressViewTab === 'requests' ? '#fff' : 'var(--text-muted)',
                    border: 'none',
                    borderRadius: '6px',
                    padding: '6px 12px',
                    fontSize: '12px',
                ...
                >
                  Live Request Telemetry Stream ({egressStats.recentRequests?.length || 0})
                </button>
              </div>

              {/* Subtab 1: Daily History */}
              {egressViewTab === 'history' && (
                <div style={{ overflowX: 'auto' }}>
                  <table className="table" style={{ width: '100%', fontSize: '12.5px' }}>
                    <thead>
                      <tr style={{ background: 'var(--bg-card-alt, #f8fafc)', textAlign: 'left' }}>
                        <th style={{ padding: '8px 12px' }}>Date</th>
                        <th style={{ padding: '8px 12px' }}>Egress Bandwidth</th>
                        <th style={{ padding: '8px 12px' }}>Share of 5GB Limit</th>
                        <th style={{ padding: '8px 12px' }}>Pace Evaluation</th>
                      </tr>
                    </thead>
                    <tbody>
                      {Object.entries(egressStats.dailyHistory || {})
                        .sort((a, b) => b[0].localeCompare(a[0]))
                        .map(([dateStr, bytes]) => {
                          const mb = bytes / (1024 * 1024);
                          const pct = (bytes / egressStats.limitBytes) * 100;
                          const isSpike = mb > 600;
                          const isHigh = mb > 250;
                          return (
                            <tr key={dateStr} style={{ borderBottom: '1px solid var(--border-color)' }}>
                              <td style={{ padding: '8px 12px', fontWeight: 600 }}>{dateStr}</td>
                              <td style={{ padding: '8px 12px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                  <div
                                    style={{
                                      width: `${Math.min(100, (mb / 1220) * 100)}%`,
                                      maxWidth: '120px',
                                      minWidth: '4px',
                                      height: '6px',
                                      background: isSpike ? '#ef4444' : isHigh ? '#f59e0b' : '#0284c7',
                                      borderRadius: '3px'
                                    }}
                                  />
                              ...
                              <td style={{ padding: '8px 12px' }}>{pct.toFixed(2)}%</td>
                              <td style={{ padding: '8px 12px' }}>
                                <span
                                  style={{
                                    fontSize: '11px',
                                    ...
                                    padding: '2px 6px',
                                    borderRadius: '4px',
                                    background: isSpike ? '#fee2e2' : isHigh ? '#fef3c7' : '#dcfce7',
                                    color: isSpike ? '#dc2626' : isHigh ? '#d97706' : '#16a34a'
                                  }}
                                >
                                  {isSpike ? 'CRITICAL SPIKE (>1GB)' : isHigh ? 'ELEVATED PACE' : 'NORMAL RANGE'}
                                </span>
                              </td>
                            </tr>
                          );
              ...
              {/* Subtab 2: Bandwidth by Table */}
              {egressViewTab === 'tables' && (
                <div style={{ overflowX: 'auto' }}>
                  <table className="table" style={{ width: '100%', fontSize: '12.5px' }}>
                    <thead>
                      <tr style={{ background: 'var(--bg-card-alt, #f8fafc)', textAlign: 'left' }}>
                        <th style={{ padding: '8px 12px' }}>Database Table / Service</th>
                        <th style={{ padding: '8px 12px' }}>Estimated Bandwidth</th>
                        <th style={{ padding: '8px 12px' }}>Share of Traffic</th>
                        <th style={{ padding: '8px 12px' }}>Active Egress Defense Strategy</th>
                      </tr>
                    </thead>
                    <tbody>
                      {Object.entries(egressStats.tableBreakdown || {})
                        .sort((a, b) => b[1] - a[1])
                        .map(([tbl, bytes]) => {
                          const totalAll = Object.values(egressStats.tableBreakdown).reduce((s, c) => s + c, 0) || 1;
                          const pct = (bytes / totalAll) * 100;
                          return (
                            <tr key={tbl} style={{ borderBottom: '1px solid var(--border-color)' }}>
                              ...
                              <td style={{ padding: '8px 12px', fontSize: '11.5px', color: 'var(--text-muted)' }}>
                                {tbl === 'saved_records'
                                  ? '✓ Metadata-first conditional delta sync active (skips 99% download when up-to-date)'
                                  : tbl === 'repair_usage_records'
                                  ? '✓ Background batched sync with 250-row chunks'
                                  : tbl === 'inventory_units'
                                  ? '✓ Site-restricted RLS scoping & debounced scan buffers'
                                  : '✓ Standard column-restricted PostgREST querying'}
                              </td>
                            </tr>
                          );
                        })}
              ...
              {/* Subtab 3: Live Request Stream */}
              {egressViewTab === 'requests' && (
                <div>
                  {(!egressStats.recentRequests || egressStats.recentRequests.length === 0) ? (
                    <div style={{ padding: '24px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '13px' }}>
                      No network requests captured in this session yet. Interact with the application or trigger a sync to inspect live byte counters.
                    </div>
                  ) : (
                    <div style={{ overflowX: 'auto', maxHeight: '280px', overflowY: 'auto' }}>
                      <table className="table" style={{ width: '100%', fontSize: '12px' }}>
                        <thead>
                          <tr style={{ background: 'var(--bg-card-alt, #f8fafc)', textAlign: 'left', position: 'sticky', top: 0 }}>
                            <th style={{ padding: '6px 10px' }}>Time</th>
                            <th style={{ padding: '6px 10px' }}>Method</th>
                            <th style={{ padding: '6px 10px' }}>Endpoint / Table</th>
                            <th style={{ padding: '6px 10px' }}>Response Egress</th>
                            <th style={{ padding: '6px 10px' }}>Status</th>
                          </tr>
                        </thead>
                        <tbody>
                          {egressStats.recentRequests.map(req => (
                            <tr key={req.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                              <td style={{ padding: '6px 10px', color: 'var(--text-muted)' }}>
                                {new Date(req.timestamp).toLocaleTimeString()}
                              </td>
                              <td style={{ padding: '6px 10px', fontWeight: 700 }}>
                                <span style={{ color: req.method === 'GET' ? '#0284c7' : '#10b981' }}>{req.method}</span>
                              </td>
